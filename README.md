...
| 하고 싶은 것 | 어떻게 | 호출 |
|------|------|------|
| **조회·질의** ("X 뭐야 / 누가 쓰나 / X 바꾸면 영향? / X와 Y 관계? / 어디에 구현?") | 평문으로 질문 — MCP 가 SSOT 를 조회해 근거와 함께 답한다 | 자동 |
...
MCP 는 다음 순서로 소스 설정을 찾아 먼저 발견되는 것을 쓴다.

| 우선순위 | 방식 | 지정 |
|------|------|------|
| 1 | `SSOT_SOURCES` (env 인라인 JSON) | 셸 `export SSOT_SOURCES='{"sources":[...]}'` 또는 `settings.json` 의 env |
| 2 | `SSOT_SOURCES_FILE` (파일 경로) | 기본값 `${CLAUDE_PLUGIN_DATA}/ssot-sources.json`. `~/.claude.json` 의 `projects[<path>].mcpServers.ssot.env` 로 프로젝트별 오버라이드 |
| 3 | `<cwd>/ssot-sources.json` | 현재 작업 디렉토리의 파일 |

가장 쉬운 방법은 `${CLAUDE_PLUGIN_DATA}/ssot-sources.json` 을 만드는 것 — 별도 env 없이 기본값으로 잡히고, 업데이트 후에도 유지된다. 프로젝트별로 다른 파일을 쓰려면 `~/.claude.json` 에서 경로를 지정한다.
...
| `${HOME}` | 홈 디렉토리 |
| `${VAR:-default}` | `VAR` 가 없으면 `default` |

> 틸드(`~`) 는 확장하지 않는다 — `${HOME}` 을 쓴다.
...
| `skills/ssot/` | 작성·검증 스킬 — init / add / fill / coverage / verify / propose / ingest / sync-lifecycle |
...
평문 질문이 오면 MCP 가 등록된 소스를 읽어 자동으로 답한다. 등록 방법은 위 "[데이터 소스 등록](#데이터-소스-등록-사용자별)" 참고.
