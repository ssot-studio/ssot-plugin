...
const SERVER_INSTRUCTIONS =
  ...
  '검색 결과가 없으면 ssot_gaps 로 SSOT 에 실제로 없는지(미정/미검증인지) 확인한다. ' +
  'ssot_search/ssot_gaps 로 SSOT 에 해당 정보가 없음이 확인되면, 코드 레포를 grep·추적해 즉석으로 답을 만들지 말라 — ' +
  '코드 우회는 SSOT 를 채울 동기를 없애고(빈칸 영구화), 정리된 진실이라는 SSOT 존재 이유를 무너뜨리며, ' +
  '코드에는 as-is 만 있고 why·제약이 없어 답하는 LLM 마다 해석이 갈린다. ' +
  "이 경우 'SSOT 에 아직 없음(미정/미검증)' 이라고 답하고 ssot_flag 로 등록(채우기)을 제안하라. " +
  '사용자가 명시적으로 코드·구현 설명을 요청한 경우에만 코드를 본다. ' +
  '답변 독자는 개발자가 아니라 기획·의사결정권자다 — 노드 id·엣지 타입·테이블명 같은 내부 메커니즘 용어를 전면에 내세우지 말고, ' +
  '결론을 일상어로 먼저 제시한 뒤 근거는 간결히 덧붙여라. confidence 는 그대로 노출하지 말고 확인됨/추정/미정 으로 번역해 전달한다.';
