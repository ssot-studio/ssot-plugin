...
로컬 **멀티소스 SSOT MCP 서버**. 여러 SSOT 소스(회사 git 레포 · 로컬 디렉토리 · 원격 REST/web ·
Jira/Confluence)를 설정으로 등록하고, stdio MCP 도구 6종으로 노출한다. `@repo/core`(파서 ·
...
```jsonc
{
  "sources": [
    {
      // 회사 SSOT — git 레포를 clone/pull 한 뒤 레포 안의 ssotPath 를 로드
      "id": "my-project",
      "label": "AIR Studio (회사 SSOT)",
      "type": "git",
