...
const fatal = out.schemaErrors.length + out.duplicateIds.length + out.danglingEdges.length + (cat.parseErrors?.length || 0);
L.push('## 요약');
L.push('');
L.push('| 검사 | 분류 | 건수 |');
L.push('|------|------|------|');
...
console.log(`  치명(구조): parse=${cat.parseErrors?.length || 0} schema=${out.schemaErrors.length} dupId=${out.duplicateIds.length} dangling=${out.danglingEdges.length}`);
